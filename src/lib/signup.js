...
export default () => {
	...
	function verification() {
		...
		if (
			...
			numbers.test(newPassword) == false ||
			lowerCaseLetters.test(newPassword) == false ||
			upperCaseLetters.test(newPassword) == false
		) {
			msjVerification.classList.remove('hide');
			msjVerification.textContent =
				'Password must contain at least 6 characters, one number and one letter in uppercase.';
		} else {
	...
	signupForm.addEventListener('submit', async (e) => {
		...
		if (user != null) {
			db.collection('users').doc(user.uid).set({
				name: nameForm,
				email: user.email,
				photoURL: 'lib/images/pic2.jpg', //foto por default
				uid: user.uid,
			});
		}
	});
